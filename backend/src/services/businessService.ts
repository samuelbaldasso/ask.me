...
export async function getPlaceStats(userId: string, placeId: string) {
  ...
  const [viewsTotal, clicksTotal, viewsWindow, clicksWindow] = await Promise.all([
    prisma.placeEvent.count({ where: { placeId, type: 'view' } }),
    prisma.placeEvent.count({ where: { placeId, type: 'click' } }),
    prisma.placeEvent.count({ where: { placeId, type: 'view', createdAt: { gte: since } } }),
    prisma.placeEvent.count({ where: { placeId, type: 'click', createdAt: { gte: since } } }),
  ]);
  ...
  return {
    ...
    viewsTotal,
    clicksTotal,
    viewsLast30Days: viewsWindow,
    clicksLast30Days: clicksWindow,
  };
}
...
export async function trackPlaceEvent(placeId: string, type: 'view' | 'click'): Promise<void> {
