...
import { approveClaim, listPendingClaims, rejectClaim } from '@/lib/api/endpoints';
      ...
      {claims.length === 0 ? (
        ...
        <div className="flex flex-col gap-3">
          {claims.map((claim) => (
            <div
              key={claim.id}
              className="flex flex-col gap-3 rounded-[24px] bg-white p-5 shadow-[0_6px_16px_rgba(124,58,237,0.06)] sm:flex-row sm:items-center sm:justify-between"
            >
              <div>
                ...
                <p className="mt-1 text-sm text-[#171123]/70">
                  Pedido de <strong>{claim.user.name ?? claim.user.email}</strong> (
                  {claim.user.email})
                </p>
              </div>
